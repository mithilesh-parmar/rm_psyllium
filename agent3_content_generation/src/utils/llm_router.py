"""LLM router: Groq (free tier) for draft generation, OpenAI GPT-4o-mini for schema polish."""
import hashlib
import os
...
def call_llm_text(
    ...
    """Call LLM and return raw text output. model: 'openai' | 'groq'"""
    actual_model = "gpt-4o-mini" if model == "openai" else "llama-3.3-70b-versatile"
    key = _cache_key(actual_model + ":text", system_prompt, user_prompt)
